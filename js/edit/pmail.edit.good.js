...
(function ($) 
{
    ...
    $.wysiwyg.controls.image = 
    {
        ...
        {
            var self = this, elements, adialog, formImageHtml, regexp, dialogReplacements, key, translation,
            img = {
                alt: "",
            ...
            var _dialog = $('<div class="pm-upload-dialog">\
<div class="pm-dialog-head">\
    <ul class="pm-page-navi">\
        <li class="item local current" id="local">本地图片</li>\
        <li class="item web" id="web">网络图片</li>\
    </ul>\
</div>\
<div class="pm-dialog-content">\
    <div class="pm-photo"><img></div>\
    <div class="pm-local-img ctrl">\
        <div><input type="file" id="pm-upload-file"/></div>\
        <label>上传本地图片</label>\
    </div>\
    <div class="pm-web-img ctrl">\
        <div><input type="text" id="pm-web-img" value="http://"/></div>\
        <label>添加网络图片，请将图片网址复制到下面文本框中</label>\
    </div>\
</div>\
<div class="pm-dialog-foot pm-ctrl"><a id="cancel" class="pm-gray-button">取消</a><a id="ok" class="pm-light-button">上传</a></div>\
            ...
            $('.pm-web-img', _dialog).hide();
            $('.pm-page-navi .item', _dialog).click(function()
            {
                if( $(this).hasClass('current') )
                    return;
                $('.pm-' + $('.current', _dialog).attr('id') + '-img').hide();
                $('.current').removeClass('current');
                $(this).addClass('current');
                $('.pm-' + $('.current', _dialog).attr('id') + '-img').show();
            });
            
            $('.pm-dialog-foot a', _dialog).click(function()
            {
                $('.insertImage').removeClass('active');
            });
        },
...
//                    if (translation === dialogReplacements[key]) 
//                    { // if not translated search in dialogs 
