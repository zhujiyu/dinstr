...
<div class="pm-user pm-user-main" id="{$target_user.ID}">
    <div class="pm-user-avatar pm-inline-block">
        ...
        <div class="pm-content-border"></div>
        <div class="pm-object-list pm-channel-list">
            {section name=joi loop=$join_channels}
                {include file="chan/pmail.channel.tpl" channel=$join_channels[joi]}
                {if !$smarty.section.joi.last}<div class="pm-border-line"></div>{/if}
            {/section}
            {*if $taget_user.join_channel_num > 40}<div class="pm-mail-more">更多</div>{/if*}
        </div>
