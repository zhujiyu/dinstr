...
class DisChanApplicantData extends DisDBTable
{
    ...
    function insert($user_id, $channel_id, $reason)
    {
    ...
    function accept()
    {
        if( !$this->ID )
            throw new DisParamException("参数不合法！");
        $str = "update channel_applicants set status = 'accept' where ID = $this->ID";
        return parent::query($str) == 1;
    }
    ...
    function refuse()
    {
        if( !$this->ID )
            throw new DisParamException("参数不合法！");
        $str = "update channel_applicants set status = 'refuse' where ID = $this->ID";
        return parent::query($str) == 1;
    }

    static function list_applicants($channel_id, $page = 0, $count = 40)
    {
        $str = "select ID, channel_id, user_id, `reason`, status, apply_time
	        from channel_applicants where channel_id = $channel_id
            order by ID desc limit ".$page*$count.", $count";
	    return parent::load_datas($str);
    }

    static function list_apply_channel_applicants($channel_id)
    {
        $str = "select ID, channel_id, user_id, `reason`, status, apply_time
	        from channel_applicants where channel_id = $channel_id and status = 0";
	    return parent::load_datas($str);
    }
    ...
    {
        $str = "select ID, channel_id, user_id, `reason`, status, apply_time
	        from channel_applicants where user_id = $user_id order by ID desc";
	    return parent::load_datas($str);
    }
}
?>