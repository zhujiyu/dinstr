...
//        pmRowMemcached::set_channel_data($this->ID, $this->detail);
    }
...
//        pmVectorMemcached::set_channel_flows($channel_id, $flag, $period, null);
        ...
        if( !$vflows )
        {
            $flows = DisNoteFlowData::load_value_mails($channel_id, $end - $period, $end);
...
//        pmVectorMemcached::set_channel_flow_ids($this->ID, null);
        ...
        {
            $flow_ids[0] = '#E#';
            $flows = DisNoteFlowCtrl::top_channel_all_flows($this->ID, $hope);
            $len = count($flows);
            for( $i = 0; $i < $len; $i ++ )
        ...
        else if( $flow_ids[0] != "#E#" )
        {
            ...
            if( $len1 < $hope )
            {
                $flows = DisNoteFlowCtrl::list_channel_all_flows($this->ID, $flow_ids[$len1 - 1], $hope - $len);
                $len2 = count($flows);
                for( $i = 0; $i < $len2; $i ++ )
